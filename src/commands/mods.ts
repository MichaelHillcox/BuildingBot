...
import curse, { Curse, Mod } from '../services/curse';
import Command from './Command';
import modinfo from '../services/modinfo';
import { Addon, File, SortableGameVersion } from '../services/CurseResponse';
...
export default class Mods implements Command {
  ...
  async curseMessage(msg: Message, mod: string, type: string) {
    ...
    const firstValid = (e: SortableGameVersion[]) =>
      e.find((a: SortableGameVersion) => a.gameVersion.includes('1.'));
    ...
    if (type === 'files') {
      const versionedFiles: { [key: string]: File & { version: string } } = {};
    ...
    if (type === 'info') {
      ...
      if (files == null) {
        msg.channel.send('Files not found... :eyes:');
        return;
      }

      const totalFiles = files.reduce((total) => total + 1, 0);
  ...
  getLinks(selectedMod: Mod, modInfo: Addon) {
    const { github } = modinfo[selectedMod.short];
